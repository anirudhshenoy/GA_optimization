...
c_const=[0.1,0.2]
alpha_const=[0,3]
airfoil_const=[0,636]          #1468 for General AFs
l_t_const=[0.1,0.7]             #From CG to C/4_t
i_t_const=[-10,4]                 #Tail setting angle
E_0=0                           #Downwash angle
...
wb2 = openpyxl.load_workbook('C:/Users/Aniru_000/Desktop/TD-1/Airfoil/s1223/airfoil/AllPolars/airfoilpolars_master.xlsx')
print ("Opened File")
...
def fitness(pop):
    ...
    cm_fit=70*math.exp(-(((CM_cg-CM_cg_const)*100)**2)/(2*7**2))                            #Gaussian function centered around lift_constant, A controls height
...
b =np.random.choice(np.arange(b_const[0],b_const[1],step=0.1),size=(params[0],1),replace=True)
c =np.random.choice(np.arange(c_const[0],c_const[1],step=0.1),size=(params[0],1),replace=True)
